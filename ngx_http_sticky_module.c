...
static ngx_int_t ngx_http_upstream_get_least_conn_peer( ngx_peer_connection_t *pc, void *data )
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, pc->log, 0, "[sticky/get_least_conn_peer] get least conn peer, try: %ui", pc->tries);
    ...
    for(peer = peers->peer, i = 0;
            peer;
            peer = peer->next, i++) {
        ngx_log_debug(NGX_LOG_DEBUG_HTTP, pc->log, 0, "[sticky/get_least_conn_peer] peer no: %ui peer conns: %ui peer weight: %ui", i,  peer->conns, peer->weight );
    ...
    if( NULL == best ) {
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, pc->log, 0, "[sticky/get_least_conn_peer] no least conn peer found");
        if( peers->next ) {
            ngx_log_debug0(NGX_LOG_DEBUG_HTTP, pc->log, 0, "[sticky/get_least_conn_peer] get least conn peer, backup servers");
    ...
    for( i = 1; i < cf->args->nelts; i++ ) {
        ...
        if( (u_char *)ngx_strstr(value[i].data, "lb_alg=") == value[i].data ) {
            ...
            if( value[i].len <= (sizeof("lb_alg=") - 1 )) {
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "a value must be provided to \"lb_alg=\"");
